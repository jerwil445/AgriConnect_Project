...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        // Initialize messaging if pre-rendered
        if (typeof window.initializeMessaging === 'function') {
            window.initializeMessaging();
        }
        
        // Handle session-based selection if it differs from pre-rendered one
        @if(session('selected_transaction_id'))
            const sessionId = {{ session('selected_transaction_id') }};
            const activeItem = document.querySelector(`.conversation-item[data-transaction-id="${sessionId}"]`);
            if (activeItem && !activeItem.classList.contains('bg-indigo-100')) {
                activeItem.click();
            }
        @endif
    });
