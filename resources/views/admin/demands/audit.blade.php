            ...
            <div class="max-w-7xl mx-auto">
                <div class="bg-white rounded-lg shadow p-6">
                    <div class="flex justify-between items-center mb-6">
                        <h2 class="text-2xl font-bold text-gray-800">Audit Matches for
                            "{{ $demand->product_name ?: $demand->egg_type }}"</h2>
                    ...
                    <div class="mb-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mt-2">
                            <div class="bg-gray-50 p-3 rounded">
                                ...
                                <p class="text-sm text-gray-900">{{ $demand->product_name ?: $demand->egg_type }}</p>
                            </div>
                            <div class="bg-gray-50 p-3 rounded">
                            ...
                            <div class="bg-gray-50 p-3 rounded">
                                ...
                                <p class="text-sm text-gray-900">PHP
                                    {{ number_format((float) ($demand->target_price ?? 0), 2) }}</p>
                            </div>
                            <div class="bg-gray-50 p-3 rounded">
                                <label class="block text-sm font-medium text-gray-600">Buyer</label>
                                <p class="text-sm text-gray-900">{{ $demand->buyer->first_name }}
                                    {{ $demand->buyer->last_name }}</p>
                            </div>
                        </div>
                    </div>
                                ...
                                @forelse($demand->matches as $match)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                            <div>{{ $match->product->product_name ?: $match->product->egg_type ?: 'N/A' }}
                                            </div>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                            {{ $match->product->quantity }} {{ $match->product->unit }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">PHP
                                            {{ number_format((float) $match->product->price, 2) }}/{{ $match->product->unit }}
                                        </td>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                            {{ $match->matched_date ? $match->matched_date->format('M d, Y') : 'N/A' }}
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            <div class="flex space-x-2">
                                                @if ($match->status == 'Pending')
                                                    <form action="{{ route('admin.matches.accept', $match) }}"
                                                    ...
                                                    <form action="{{ route('admin.matches.reject', $match) }}"
                                                        method="POST" class="inline">
                                                        @csrf
