...
<div class="container mx-auto px-4 py-10 max-w-6xl">
    ...
    <div class="relative mb-8">
        {{-- Hero Background --}}
        <div class="h-48 w-full bg-gradient-to-r from-blue-600 to-indigo-700 rounded-t-3xl shadow-lg relative overflow-hidden">
        ...
        <div class="bg-white rounded-b-3xl shadow-xl border-x border-b border-gray-100 p-8 flex flex-col md:flex-row items-center md:items-end gap-6 -mt-16 relative z-10">
            <div class="relative">
                ...
                <div class="absolute -bottom-2 -right-2 w-10 h-10 bg-blue-500 rounded-2xl border-4 border-white flex items-center justify-center text-white shadow-lg">
            ...
            <div class="flex-1 text-center md:text-left">
                ...
                <div class="flex flex-wrap justify-center md:justify-start gap-3 mt-4">
                    <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-lg bg-green-50 text-green-700 text-xs font-bold border border-green-100">
            ...
            <div class="flex flex-col gap-3 min-w-[200px]">
                ...
                <div class="w-full bg-gray-100 h-2.5 rounded-full overflow-hidden border border-gray-100">
                    <div class="bg-gradient-to-r from-blue-500 to-indigo-600 h-full rounded-full transition-all duration-[1500ms]" style="width: {{ $completeness }}%"></div>
                </div>
                @if($completeness < 100)
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <div class="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm hover:shadow-md transition-all group">
            <div class="flex items-center gap-4">
                <div class="w-14 h-14 bg-purple-50 rounded-2xl flex items-center justify-center text-purple-600 text-xl group-hover:scale-110 transition-transform">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
                ...
                <div class="p-8">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div>
                            <label class="block text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-2">Company Entity</label>
                            <p class="text-gray-900 font-bold bg-gray-50 px-4 py-3 rounded-xl border border-gray-100">{{ Auth::user()->buyer->company_name ?? 'Not specified' }}</p>
                        </div>
                        <div>
                            <label class="block text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-2">Business Sector</label>
                            <p class="text-gray-900 font-bold bg-gray-50 px-4 py-3 rounded-xl border border-gray-100">{{ Auth::user()->buyer->business_type ?? 'Not specified' }}</p>
                        </div>
                        <div class="md:col-span-2">
                            <label class="block text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-2">Sourcing Interests</label>
                            <div class="flex flex-wrap gap-2">
                                ...
                                @forelse($interests as $interest)
                                    @if(trim($interest))
                                        <span class="px-4 py-2 bg-blue-50 text-blue-700 rounded-xl font-bold text-sm border border-blue-100">{{ trim($interest) }}</span>
                                    @endif
                                @empty
                                    <p class="text-gray-400 italic font-medium p-4 bg-gray-50 w-full rounded-xl border border-dashed border-gray-200">No interests specified yet.</p>
                                @endforelse
                            </div>
                        </div>
                        <div class="md:col-span-2">
                            <label class="block text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-2">Operational Hub</label>
                            <div class="flex items-start gap-3 bg-gray-50 p-4 rounded-xl border border-gray-100">
                                ...
                                <p class="text-gray-900 font-bold leading-relaxed">{{ Auth::user()->buyer->address ?? 'No address registered' }}</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
                ...
                <div class="p-8">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div>
                            <label class="block text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-2">Mobile Connectivity</label>
                            <p class="text-gray-900 font-bold">{{ Auth::user()->phone_number ?? 'No number listed' }}</p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="lg:col-span-1 space-y-6">
            <div class="bg-gray-900 rounded-3xl p-8 text-white shadow-2xl relative overflow-hidden group">
                <div class="absolute -right-10 -top-10 w-32 h-32 bg-blue-500/20 rounded-full blur-3xl group-hover:scale-150 transition-transform duration-700"></div>
                ...
                <div class="mt-8 pt-8 border-t border-white/5">
                    <p class="text-[10px] font-bold text-gray-500 uppercase tracking-widest leading-loose">
                        Your profile is visible to verified farmers during the matching process. Keep it updated for better trust.
                    </p>
                </div>
            </div>

            <div class="bg-gradient-to-br from-green-400 to-emerald-600 rounded-3xl p-8 text-white shadow-xl relative overflow-hidden">
                ...
                <p class="text-sm font-medium opacity-90 leading-relaxed mb-6 italic">The more detailed your sourcing interests, the better farmers can serve you.</p>
                <div class="bg-white/20 backdrop-blur-md rounded-2xl p-4 border border-white/30 text-center">
                    ...
                    <div class="text-[10px] font-black uppercase tracking-tighter opacity-80 mt-1">Tier 1 Recognition</div>
                </div>
            </div>
        </div>
    </div>
